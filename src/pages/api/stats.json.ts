...
export const GET: APIRoute = async () => {
  ...
  const apiKey = env('UMAMI_API_KEY'); // Umami Cloud (paid API key)
  const bearer = env('UMAMI_API_TOKEN'); // self-hosted bearer token

  // NB: the free-plan "Share URL" does NOT expose a raw-stats API; inline counts
  // need a paid Cloud API key or a self-hosted token. Without them the widget
  // gracefully falls back to a link to the public share dashboard (client-side).
  ...
  try {
    ...
    const path = apiKey ? `/v1/websites/${websiteId}/stats` : `/api/websites/${websiteId}/stats`;
    const url = `${base}${path}?startAt=${LAUNCH}&endAt=${Date.now()}`;
